    ...
    describe('with enums', () => {
      it('references enum in the right schema #3171', () => {
        expectsql(sql.createTableQuery(FooUser.getTableName(), sql.attributesToSQL(FooUser.rawAttributes), { }), {
          sqlite: 'CREATE TABLE IF NOT EXISTS `foo.users` (`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, `mood` TEXT);',
          postgres: 'CREATE TABLE IF NOT EXISTS "foo"."users" ("id"   SERIAL , "mood" "foo"."enum_users_mood", PRIMARY KEY ("id"));',
          oracle: "BEGIN EXECUTE IMMEDIATE 'CREATE TABLE foo_users (id NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY, mood NVARCHAR2(255),CONSTRAINT PKfoo_usersid PRIMARY KEY (id), CHECK (mood IN(''happy'', ''sad'')))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;",
