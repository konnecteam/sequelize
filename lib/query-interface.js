...
class QueryInterface {
  ...
  removeColumn(tableName, attributeName, options) {
   ...
   /**
   * Get the sql for adding indexes
 
   * @return {Promise}
   */
  addIndexesQuery(tableName, options, attributeName) {
    options = options || {};
    switch (this.sequelize.options.dialect) {
      case 'sqlite':
        // sqlite needs some special treatment as it cannot drop a column
        return SQLiteQueryInterface.addIndexesQuery.call(this, tableName, attributeName, options);
      default:
        return '';
    }
  }
   ...
   * @param {Object} attrOptions                    An object to define the constraint name, type etc
   * @param {String} attrOptions.type               Type of constraint. One of the values in available constraints(case insensitive)
   * @param {String} [attrOptions.name]             Name of the constraint. If not specified, sequelize automatically creates a named constraint based on constraint type, table & column names
   * @param {String} [attrOptions.defaultValue]     The value for the default constraint
   * @param {Object} [attrOptions.where]            Where clause/expression for the CHECK constraint
   * @param {Object} [attrOptions.references]       Object specifying target table, column name to create foreign key constraint
   * @param {String} [attrOptions.references.table] Target table name
   * @param {String} [attrOptions.references.field] Target column name
   *
   * @return {Promise}
   */
  addConstraint(tableName, attributes, attrOptions, rawTablename, options) {
    if (!Array.isArray(attributes)) {
      rawTablename = attrOptions;
      attrOptions = attributes;
      attributes = attrOptions.fields;
    }
    ...
    attrOptions = Utils.cloneDeep(attrOptions);
    attrOptions.fields = attributes;

    if (this.sequelize.dialect.name === 'sqlite') {
      attrOptions.transaction = options && options.transaction;
      return SQLiteQueryInterface.addConstraint.call(this, tableName, attrOptions, rawTablename);
    } else {
      const sql = this.QueryGenerator.addConstraintQuery(tableName, attrOptions, rawTablename);
      return this.sequelize.query(sql, options);
    }
  }
  ...
  select(model, tableName, options) {
    ...
    //Implementation of getting the rawQuery
    let rawQuery = this.QueryGenerator.selectQuery(tableName, options, model);
    if (options.rawQuery) {
      if (this.QueryGenerator.dialect === 'oracle' && !options.rawQuery.original) {
        const opts = require('./dialects/oracle/query').prototype._dealWithLongAliasesBeforeSelect(rawQuery);
        rawQuery = opts.sql;
      }
      return rawQuery
    } 
   
    return this.sequelize.query(rawQuery, options);
  }
