...
class ModelManager {
  ...
  forEachModel(iterator, options) {
    ...
    for (const model of this.models) {
      ...
      for (const attrName in model.rawAttributes) {
        if (model.rawAttributes.hasOwnProperty(attrName)) {
          ...
          if (attribute.references) {
            ...
            //we push this deps in node too beacause it may have been overwritten by a new "define" before the sync
      ...
      deps.forEach( dep => {
        if(tableName !== dep) {
          edges.push([tableName, dep]);
        }
      });
      if (nodes.indexOf(tableName) === -1) {
        nodes.push(tableName);
      }
    }

    sorted = toposort.array(nodes, edges);
    if (options.reverse) {
