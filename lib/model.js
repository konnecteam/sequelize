...
class Model {
  ...
  static refreshAttributes() {
    ...
    _.each(this.rawAttributes, (definition, name) => {
      ...
      } else if (definition.type && (definition.type instanceof DataTypes.JSON || definition.type.key === 'JSON')) {
    ...
    for (const option of options) {
      ...
      if (scope) {
        _.assignWith(self._scope, scope, (objectValue, sourceValue, key) => {
          if (key === 'where') {
            //We now concat the scopes between them and not override the default ones.
            let concatWhere;
            ...
            if (sourceValue) {
              if (Array.isArray(sourceValue)) {
                concatWhere = concatWhere || [];
                if (!Array.isArray(concatWhere)) {
                  //On créer un array avec les éléments de concatWhere et on ajoute ceux de sourceValue (permet de garder ceux avec la même clé)
                  const temp = [];
                  Object.keys(concatWhere).forEach(key => {
                    const obj = {};
                    obj[key] = concatWhere[key];
                    temp.push(obj);
                  })
                  concatWhere = temp;
                } 
                ...
                if (Array.isArray(concatWhere)) {
                  ...
                  concatWhere = Object.assign(concatWhere, sourceValue)
                }
              }
            }
  ...
  static bulkBuild(valueSets, options) { // testhint options:none
    ...
    if (options.attributes) {
      options._userAttributes = options.attributes; // so we can have the aliases and corresponding column
      options.attributes = options._userAttributes.map(attribute => Array.isArray(attribute) ? attribute[1] : attribute);
    }
    ...
    if (typeof key === 'object' && key !== null) {
      ...
      if (!options.raw && this._customSetters[key]) {
        ...
        if (this._options && this._options.include && this._options.includeNames.indexOf(key) !== -1) {
          ...
          //we check if the key is an alias
          if (options._userAttributes) {
            options._userAttributes.forEach(userAttribute => {
              if (userAttribute instanceof Array && userAttribute[1] === key) {
                //a t on spécifié la colonne surlequel cet alias est rattaché
                if (userAttribute.length === 3 && userAttribute[2].startsWith('JSONCOL_')) {
                  baseKey = userAttribute[2].substr(8);
                } else if (userAttribute[0] instanceof Utils.Col) {
                  // récupère la colonne d'origine pour cet alias 
                  // attention la colonne est préfixé par la table on enlève cette dernière
                  baseKey = userAttribute[0].col.substring(userAttribute[0].col.indexOf('.') + 1); 
                }
              }
            })
          }
          ...
          if (this.constructor._isJsonAttribute(baseKey)) {
            const type =  this.constructor.tableAttributes[baseKey].type; //this.sequelize.models[this._modelOptions.name.singular].attributes[baseKey].type;
            
            if (type && type.parseInSet) {
              value = type.parseInSet(value);
            }
          }
