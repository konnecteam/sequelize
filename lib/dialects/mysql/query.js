  ...
  run(sql) {
    ...
    if (this.isSelectQuery()) {
      ...
      return new Utils.Promise((resolve, reject) => {
        ...
        // TODO a passer en stream 
        // https://stackoverflow.com/questions/36015279/how-to-use-node-mysql-without-loads-all-the-rows-into-the-memory
        // Attention problème les erreurs ne remontent pas en utilisant le stream
        this.connection.query({ sql: this.sql })
        .on('result', row => {
          // mysql connector doesn't provide anything to stop the query after hitting the max rows
          if (maxRows == null || rowsToProcess.length < maxRows) {
        ...
        .on('error', sqlErr => {
          ...
          reject(this.formatError(sqlErr));
        })
        .on('end', sqlErr => {
          ...
          resolve(this.formatResults(rowsToProcess));
        });
      });
    } else {
      return new Utils.Promise((resolve, reject) => {
