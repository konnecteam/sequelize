...
const QueryGenerator = {
        ...
        if (typeof item === 'string') {
          ...
          if (index > 0 && orderIndex !== -1) {
            ...
            if (previousModel.associations !== undefined && previousModel.associations[item]) {
              ...
              const prevModelDataType = previousModel.rawAttributes[itemSplit[0]].type;
              if (prevModelDataType && (prevModelDataType instanceof DataTypes.JSON || prevModelDataType.key === 'JSON')) {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    const topLevelInfo = {
      // We need the real "top" names for the aliases
      names: options.topNames || mainTable,
      options,
      subQuery
    };
    let mainJoinQueries = [];
  ...
  escapeAttributes(attributes, options, mainTableAs) {
    return attributes && attributes.map(attr => {
      ...
      if (Array.isArray(attr)) {
        if (attr.length < 2 ) { // we can have a third param if it's a JSON column
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (include.through) {
      ...
      // will ignore subqueries linked to hasMany despite having offset set
      const ignoreAllSubQueries = topLevelInfo.options.ignoreAllSubQueries;
      if (!ignoreAllSubQueries && !(Model.parentExistInLimitQuery(include)) && notFromBTM && include.required === true && !(include.mismatch) && (hasLimitRequiredHasMany || (topLevelInfo.subQuery && include.subQueryFilter))) {
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    if (include.where || include.through.where) {
      ...
      if (topLevelInfo.subQuery && include.required) {
        ...
        const foundIndex = include.include.indexOf(topInclude);
        if (foundIndex > -1){
          include.include.splice(foundIndex, 1);
        } 
    ...
    if (smth instanceof Utils.Where) {
      ...
      if (value && value instanceof Utils.SequelizeMethod) {
        ...
        if (typeof value === 'boolean') {
          ...
          //smth.comparator === 'BETWEEN' ||
          if (smth.comparator === 'IN' || smth.comparator === 'NOT IN') {
            let tpmValue = '('
            value.forEach( (val, index) => {
              tpmValue += this.escape(val);
  ...
  whereItemQuery(key, value, options) {
    ...
    if (key && typeof key === 'string' && key.indexOf('.') !== -1 && options.model) {
      ...
      const rawAttr = options.model.rawAttributes[keyParts[0]];
      if (rawAttr && rawAttr.type && (rawAttr.type instanceof DataTypes.JSON || rawAttr.type.key === 'JSON')) {
        const tmp = {};
        const field = rawAttr;
    ...
    if (isPlainObject && fieldType && (fieldType instanceof DataTypes.JSON || fieldType.key === 'JSON') && options.json !== false) {
