...
module.exports = BaseTypes => {
 ...
 /* function JSON() {
  ...
  inherits(JSON, BaseTypes.JSON);

  JSON.prototype.toSql = function toSql() {
    return 'CLOB';
  };
*/
  function JSONTYPE() {
  ...
  JSONTYPE.prototype.toSql = function toSql() {
    return 'CLOB';
  };

  // /!\ JSON type don't exist in metadata of column,
  // thus we can't use "parse"
  // JSONTYPE.parse = function parse(data) {
  //   return JSON.parse(data);
  // };

  JSONTYPE.prototype.stringify = function(data) {
    ...
    if (this._length) {
      if (typeof this._length === 'string') {
        switch (this._length.toLowerCase()) {
          ...
          case 'medium' : 
            warn('ORACLE does not support TEXT with the `length` = `medium` option. `CLOB` will be used instead.');
            return 'CLOB';
          case 'long' : 
            warn('ORACLE does not support TEXT with the `length` = `long` option. `CLOB` will be used instead.');
            return 'CLOB';
        }
      }
