...
const QueryGenerator = {
  ...
  dialect: 'oracle',
   ...
   /**
   * Warning oracle schema ≠ mssql schema or postgres schema DO NOT USE
   */
  createSchema(schema) {
  ...
  /**
   * Warning oracle schema ≠ mssql schema or postgres schema
   */
  showSchemasQuery() {
    return 'SELECT USERNAME AS "schema_name" FROM ALL_USERS WHERE ORACLE_MAINTAINED = (\'N\') AND USERNAME != user';
  },
  ...
  // We take the specified schema delimiter or '_' by default
  addSchema(param) {

    if (!param._schema) return param.tableName || param;

    const schema = super.addSchema(param);
    schema.delimiter = param._schemaDelimiter || '_';
    return schema;
  },
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const jsonRegex = /ISJSON \(([a-zA-Z_.0-9]*) (.*)\)/g; //Check regex
    ...
    for (let attr in attributes) {
      if (attributes.hasOwnProperty(attr)) {
        ...
        } else if (_.includes(dataType, 'ISJSON')) {
          //Check constraints go to the end
          match = dataType.match(/^(.+) (ISJSON.*)$/);
          attrStr.push(attr + ' ' + match[1]);

          const checkCond = `CONSTRAINT ${tableName}_${attr}_isjson_chk CHECK( ${attr} IS JSON)`;
  ...
  describeTableQuery(tableName, schema, schemaDelimiter) {
    ...
    const sql = ['SELECT atc.COLUMN_NAME, atc.DATA_TYPE, atc.DATA_DEFAULT, atc.DATA_LENGTH, atc.CHAR_LENGTH, atc.DEFAULT_LENGTH, atc.NULLABLE, ',
      'CASE WHEN ucc.CONSTRAINT_NAME  LIKE \'%PK%\' THEN \'PRIMARY\' ELSE \'\' END AS "PRIMARY" ',
      'FROM all_tab_columns atc ',
      'LEFT OUTER JOIN all_cons_columns ucc ON(atc.TABLE_NAME = ucc.TABLE_NAME AND atc.COLUMN_NAME = ucc.COLUMN_NAME and atc.OWNER = ucc.OWNER ) ',
    ...
    let currTableName = _.isPlainObject(tableName) ? tableName.tableName : tableName;
    currTableName = schema ? schemaDelimiter ? schema + schemaDelimiter + currTableName : schema + '_' + currTableName : currTableName
    const values = {
    ...
    jsonPathExtractionQuery(column, path) {
      const paths = _.toPath(path).map( f => {
        if (/^\w+$/.test(f) === false) {
          return `"${f}"`;
        } else {
          return f;
  ...
  //Manque la méthode pour gérer le JSON depuis un WHERE
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    if (smth instanceof Utils.Json) {
      // Parse nested object
      if (smth.conditions) {
        const conditions = _.map(this.parseConditionObject(smth.conditions), condition =>
          `JSON_VALUE(${this.quoteIdentifier(_.first(condition.path))}, '\$.${_.tail(condition.path).join('.')}') = '${condition.value}'`
        );
        ...
        if (this._checkValidJsonStatement(smth.path)) {
          ...
          str = `JSON_VALUE(${this.quoteIdentifier(columnName)}, '\$${startWithDot ? '.' : ''}${path.join('.')}')`;
        }
      ...
      if (/timestamp/i.test(smth.type)) {
        ...
        smth.type = 'integer';
      } else if (/double precision/i.test(smth.type)) {
        smth.type = 'decimal';
  ...
  showConstraintsQuery(tableName) {
    return `SELECT constraint_name "constraintName", constraint_type "constraintType" , column_name "columnName" from user_constraints natural join user_cons_columns where table_name = '${tableName.toUpperCase()}'`;
  },

  showTablesQuery() {
    return 'SELECT owner as table_schema, table_name, 0 as lvl FROM all_tables where OWNER IN (SELECT USER FROM DUAL)';
  },
  ...
  changeColumnQuery(tableName, attributes) {
    ...
    //if we have a default null we keep it
    const secondQuery = query.replace('DEFAULT NULL', 'REPLACEDEFAULTVAL').replace('NOT NULL', '').replace('NULL', '').replace('REPLACEDEFAULTVAL', 'DEFAULT NULL');
    ...
    for (key in valueHash) {
      if (valueHash.hasOwnProperty(key)) {
        ...
        if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && !value) {
          ...
          if (modelAttributeMap && modelAttributeMap[key] && !modelAttributeMap[key].allowNull && (value != null && value && !(value instanceof Array) && value.length === 0)) {
          ...
          if (currAttribute && currAttribute.type != null && (currAttribute.type.key === DataTypes.TEXT.key || currAttribute.type.key === DataTypes.BLOB.key)) {
            ...
            //Gestion de l'utilisation des binding dans la fonction execute d'oracle-db (pour les CLOB etc ...)
            const paramName = this._handleBindingParameters(key, inputParamCpt, valueHash, modelAttributes, inputParameters);
  ...
  */
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    for (const key in attrValueHash) {
      ...
      const currAttribute = modelAttributeMap[key];
      if (currAttribute && currAttribute.type != null && (currAttribute.type.key === DataTypes.TEXT.key || currAttribute.type.key === DataTypes.BLOB.key)) {
        //If we try to update into TEXT or BLOB, we need to pass by input-parameters to avoid the 4000 char length limit
        //Gestion de l'utilisation des binding dans la fonction execute d'oracle-db (pour les CLOB etc ...)
        const paramName = this._handleBindingParameters(key, inputParamCpt, attrValueHash, attributes, inputParameters);
  ...
  _handleBindingParameters(key, inputParamCpt, attrValueHash, attributes, inputParameters) {
    ...
    if (attributes[key].type.key === DataTypes.TEXT.key) {
      //if text with length, it's generated as a String inside Oracle, 
      if (attributes[key].type._length !== '') {
    ...
    if (allAttributes.length > 0) {
      ...
      _.forEach(attrValueHashes, (attrValueHash, idx, array) => {
        ...
        const attrs = allAttributes.map(key => {
          let currAttribute = key in attributes ? attributes[key] : key;
          ...
          } else if (currAttribute && currAttribute.type != null && (attributes[key].type instanceof DataTypes.JSON || attributes[key].type.key === 'JSON')) {
            return this.escape(JSON.stringify(attrValueHash[key])) + ' AS "' + key + '"';
          } else {
  ...
  showIndexesQuery(tableName, options) {
    ...
    if (_.isPlainObject(tableName)) {
      const schema = tableName.schema ? options && options.delimiter ? tableName.schema + options.delimiter : tableName.schema + '_' : '';
    ...
    const sql = ['SELECT i.index_name,i.table_name, i.column_name, u.uniqueness, i.descend ',
      ...
      'ON (u.table_name = i.table_name AND u.index_name = i.index_name AND u.owner = i.index_owner) ',
      'WHERE (i.table_name = UPPER(\'<%= tableName %>\') OR i.table_name = \'<%= tableName %>\')',
      'AND u.OWNER = (SELECT USER FROM DUAL)',
  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      if (attribute.autoIncrement) {
        ...
        if (attribute.type && attribute.type.key === DataTypes.DOUBLE.key) {
          template = attribute.type.toSql();
        } else if (attribute.type && attribute.type.key === DataTypes.JSON.key) {
          if (options.context === 'addColumn') {
            template = ` CLOB CONSTRAINT ${options.tableName}_${options.attr}_isjson_chk CHECK( ${options.attr} IS JSON)`;
          } else {
            template = ' CLOB ISJSON';
          }
    ...
    } else if (attribute.defaultValue == null && (options && options.context === 'alterColumn')) {
        //if we change a column and their is no default value oracle must know
        if (!attribute.type || attribute.type !== 'TEXT') {
  ...
  quoteTable(param, as) {
    ...
    if (_.isObject(param)) {
      if (param.schema) {
        ...
        // Oracle schema ≠ mssql schema or postgres schema DO NOT USE
        table += param.schema + (param.delimiter || '_') + param.tableName;
  ...
  nameIndexes(indexes, rawTablename) {
    ...
    if (_.isObject(rawTablename)) {
      tableName = rawTablename.schema  + '_' + rawTablename.tableName;
    } else {
  ...
  isIdentityPrimaryKey(tableName) {
    return ['SELECT TABLE_NAME,COLUMN_NAME, COLUMN_NAME,GENERATION_TYPE,IDENTITY_OPTIONS FROM ALL_TAB_IDENTITY_COLS WHERE TABLE_NAME=\'',
      tableName.tableName ? tableName.tableName.toUpperCase() : tableName.toUpperCase(),
      '\' '].join('');
  },
