  ...
  */
    ...
    if (allAttributes.length > 0) {
      ...
      _.forEach(attrValueHashes, (attrValueHash, idx, array) => {
        ...
        const attrs = allAttributes.map(key => {
          ...
          } else if (currAttribute && currAttribute.type != null && currAttribute.type.key === DataTypes.JSON.key) {
  ...
  showIndexesQuery(tableName) {
    ...
    const sql = ['SELECT i.index_name,i.table_name, i.column_name, u.uniqueness, i.descend ',
      ...
      owner.length > 0 ? ' AND u.TABLE_OWNER = \'' + this.getOwnerToGoodCase(owner) + '\'' : 'AND u.OWNER = (SELECT USER FROM DUAL)',
