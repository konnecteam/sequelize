...
class Query extends AbstractQuery {
  ...
  */
    ...
    if (this.isInsertQuery(data)) {
      ...
      data.rows.forEach(_result => {
        ...
        if (!(_result.COLUMN_NAME.toLowerCase() in result)) {
          ...
          if (result[key].type === 'BOOLEAN') {
            result[key].defaultValue = { 'false': false, 'true': true }[result[key].defaultValue];
          ...
          if (typeof result[key].defaultValue === 'string') {
            result[key].defaultValue = result[key].defaultValue.trim().replace(/'/g, '');
          }
        }
      });
      ...
      for (let rowsIdx = 0; rowsIdx < rows.length ; rowsIdx++) {
        ...
        for (let keysIdx = 0; keysIdx < keys.length; keysIdx++) {
          ...
          if (key.indexOf('.') > -1) {
            ...
            //we check if it is prefixed with a column in our model
            if (Object.keys(attrs).includes(parts[0])) {
              //if this attributes is a JSON column we assume it is an alias containing the key retrieved by the query we can push it
              if (this.model.attributes[parts[0]].type.key === DataTypes.JSON.key) {
                newRow[key] = element[key];
              }

            } else if (this.options && this.options.includeMap) {
  ...
  formatError(err) {
    ...
    // DPI-1067: call timeout of XXX ms exceeded with ORA-3156
    match = err.message.match(/DPI-1067/);
    if (match && match.length > 0) {
