  ...
  connect(config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.lib.getConnection(connectionConfig).then(connection => {
        try { 
          //callTimeout is set on the connection directly;
          if (connectionConfig.callTimeout != null) { 
            connection.callTimeout = connectionConfig.callTimeout; 
          }
        } catch (err) {
          //if instant-client <18 it throws an error we log it but we can continue without it;
        ...
        //plus we can alter NLS_NUMERIC_CHARACTERS
        if (connectionConfig.NLS_COMP || connectionConfig.NLS_SORT || connectionConfig.NLS_NUMERIC_CHARACTERS) {
          let alterSessionQry = 'BEGIN ';
          alterSessionQry += connectionConfig.NLS_COMP ? "EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_COMP=" + connectionConfig.NLS_COMP + "';" : '';
          alterSessionQry += connectionConfig.NLS_SORT ? "EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_SORT=" + connectionConfig.NLS_SORT + "';" : '';
          alterSessionQry += connectionConfig.NLS_NUMERIC_CHARACTERS ? "EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_NUMERIC_CHARACTERS=''" + connectionConfig.NLS_NUMERIC_CHARACTERS + "''';" : '';
          alterSessionQry += ' END;' 
          ...
          switch (errorCode) {
            ...
            case 'ORA-12541' : //ORA-12541: TNS:No listener
              reject(new sequelizeErrors.AccessDeniedError(err)); 
            case 'DPI-1067':
              reject(new sequelizeErrors.TimeoutError(err));
            case 'NJS-007':
              reject(new sequelizeErrors.AccessDeniedError(err));
            default:
