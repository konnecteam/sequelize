...
const QueryGenerator = {
  ...
  showConstraintsQuery(tableName) {
    ...
    return [
      'SELECT tc.constraint_catalog AS "constraintCatalog",',
      ...
      'left outer join INFORMATION_SCHEMA.key_column_usage as "kcu" on (tc.constraint_name = kcu.constraint_name and tc.table_name = kcu.table_name)',
  ...
  addLimitAndOffset(options, model) {
    ...
    const tableName = options.tableAs || (model && model.name);
    if ((options.limit || options.offset) && tableName) {
      if (!(options.order && options.group) && (!options.order || options.include && !orders.subQueryOrder.length)) {
        fragment += (options.order && !isSubQuery) ? ', ' : ' ORDER BY ';
        fragment += this.quoteTable(tableName) + '.' + this.quoteIdentifier(model.primaryKeyField);
      }
    }
