  ...
  run(sql, parameters) {
    ...
    return query.catch(err => {
      ...
      .then(queryResult => {
        ...
        const rows = isArray? queryResult.reduce((allRows, r) => allRows.concat(r.rows || []), []) : queryResult.rows;
        const rowCount = isArray ? queryResult.reduce((count, r) => Number.isFinite(r.rowCount) ? count + r.rowCount : count, 0 ) : queryResult.rowCount || 0;
