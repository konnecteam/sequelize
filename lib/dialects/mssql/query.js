...
class Query extends AbstractQuery {
  ...
  _run(connection, sql, parameters) {
    this.sql = sql;
    const maxRows = this.isSelectQuery() && this.sequelize.options && this.sequelize.options.dialectOptions ? this.sequelize.options.dialectOptions.maxRows : 0;
  ...
  formatResults(data, rowCount) {
    ...
    if (this.isShowTablesQuery()) {
      ...
      for (const _result of data) {
        if (_result.Default) {
          _result.Default = _result.Default.replace("('", '').replace("(N'", '').replace("')", '').replace(/'/g, '');
        }
