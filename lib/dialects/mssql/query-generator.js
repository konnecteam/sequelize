...
const QueryGenerator = {
      ...
      _.forEach(attrValueHashes, attrValueHash => {
        tuples.push('(' +
          allAttributes.map(key => {
            let attrValueHashVal = attrValueHash[key];
            if (key in attributes && attributes[key].type && (attributes[key].type instanceof DataTypes.JSON || attributes[key].type.key === 'JSON')) {
              attrValueHashVal = attributes[key].type.stringify(attrValueHashVal, this);
            }
            return this.escape(attrValueHashVal);
          }).join(',') +
        ')');
      });
  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.type !== 'TEXT' && attribute.type._binary !== true &&
        Utils.defaultValueSchemable(attribute.defaultValue) && !(options && options.context === 'alterColumn')) {
